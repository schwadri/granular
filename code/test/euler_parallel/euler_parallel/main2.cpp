...
/* euler integration for rigid body with quaternion orientation.
   separate handling of translational and rotational dofs because
   the handling is so different.
 */
...
cl_mem x_mem, dx_mem, p_mem, dp_mem;
...
void setup_kernel() {
  ...
  kernel = clCreateKernel(program, "euler_split4", &error);
  check_error(error);
}
...
int main() {
  ...
  check_error(clSetKernelArg(kernel, 1, sizeof(cl_mem), (void *) &dx_mem));
  check_error(clSetKernelArg(kernel, 2, sizeof(cl_mem), (void *) &p_mem));
  check_error(clSetKernelArg(kernel, 3, sizeof(cl_mem), (void *) &dp_mem));
