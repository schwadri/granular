...
//  broadphase_2d
//
//  Created by Adrian Schweizer on 7/5/12.
...
cl_mem x_mem, p_mem, cell_body_mem;
...
__attribute__((packed)) struct cell_body {
  unsigned int cell;
  //unsigned int body;
};

size_t nbodies = 1024*1024;
cell_body * cell_bodies;
xstate  * xstates;
pstate  * pstates;

//number of global cells in x- and y- direction
unsigned int n_cx = 6, n_cy = 6;
...
void deinit_cl() {
  ...
  clReleaseMemObject(cell_body_mem);
...
void setup_kernel() {
  cl_int error;
  //load source
  std::ifstream prog_src_in("broadphase.cl");
  ...
  kernel = clCreateKernel(program, "map_body_to_cell_v4", &error);
  check_error(error);
}
...
void setup_cl_mem() {
  ...
  check_error(error);
  cell_body_mem = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(cell_body) * nbodies, NULL, &error);
  check_error(error);
}
...
void init_sys() {
  ...
  for(int i = 0; i < nbodies; ++i) {
    ...
    //distribute bodies uniformly on the grid
    x.x     = (n_cx * l_x) * real_t(rand()) / real_t(RAND_MAX);
    x.y     = (n_cy * l_y) * real_t(rand()) / real_t(RAND_MAX);
...
int main() {
  ...
  check_error(clSetKernelArg(kernel, 0, sizeof(cl_mem), (void *) &x_mem));
  check_error(clSetKernelArg(kernel, 1, sizeof(cl_mem), (void *) &p_mem));
  check_error(clSetKernelArg(kernel, 2, sizeof(cl_mem), (void *) &cell_body_mem));
  cl_float4 l_xi = {l_x, l_y, 1000.0f, 1000.0f};
  cl_uint4  n_cxi = {n_cx, n_cy, 1, 1};
  check_error(clSetKernelArg(kernel, 3, sizeof(cl_float4), &l_xi));
  check_error(clSetKernelArg(kernel, 4, sizeof(cl_uint4), &n_cxi));
