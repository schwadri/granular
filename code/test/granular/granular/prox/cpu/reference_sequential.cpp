...
reference_sequential_sor_prox::reference_sequential_sor_prox(
...
void reference_sequential_sor_prox::collider_contact_to_solver_contact(
 ...
 */
void reference_sequential_sor_prox::setup_bcsr_gij(granular_system const & sys) {
  m_gij_columns.clear();
...
void reference_sequential_sor_prox::reorder_contacts_by_colors(
  granular_system &                 sys,
...
void reference_sequential_sor_prox::setup_contacts(
  granular_system &                 sys,
  std::vector<collider::contact>  &  contacts,
  std::vector<std::vector<index_t> > &    cliques
) {
...
void reference_sequential_sor_prox::apply_percussions(granular_system & sys) {
  for(index_t i = 0; i < m_contacts.size(); ++i) {
    contact const & ci = m_contacts[i];
 ...
 */
vec3 reference_sequential_sor_prox::solve_one_contact_problem_alart_curnier(contact const & ci, vec3 pold, vec3 const & b, real tol_rel, real tol_abs) {
...
/* solves a one contact problem
 */
vec3 reference_sequential_sor_prox::solve_one_contact_problem_pseudo_enum_alart_curnier(contact const & ci, vec3 pold, vec3 const & b) {
 ...
 */
prox_result reference_sequential_sor_prox::solve_multi_contact_problem_sor() {
  bool converged          = false;
  bool diverged           = false;
  unsigned int iteration  = 0;
...
void reference_sequential_sor_prox::insert_into_independent_sets(
...
void reference_sequential_sor_prox::build_independent_contact_sets(
...
prox_result reference_sequential_sor_prox::run() {
