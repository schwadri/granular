//
//  chaotic_parallel_contact_read_write_lock.cpp
//  granular
//
...
#include "chaotic_parallel_contact_read_write_lock.hpp"

chaotic_parallel_contact_read_write_lock_sor_prox::chaotic_parallel_contact_read_write_lock_sor_prox(
...
m_max_local_iterations(max_local_iterations_), m_mutexes(0) {
  m_work.resize(m_worker_count);
  m_sub_offsets.resize(m_worker_count + 1);
}

void chaotic_parallel_contact_read_write_lock_sor_prox::insert_into_independent_sets(
...
void chaotic_parallel_contact_read_write_lock_sor_prox::build_independent_contact_sets(
  ...
  for(index_t i = 0; i < contacts.size(); ++i) {
    ...
    insert_into_independent_sets(m_colors, cliques, m_independent_sets, i, ci.key);
  }
...
void chaotic_parallel_contact_read_write_lock_sor_prox::collider_contact_to_solver_contact(
...
void chaotic_parallel_contact_read_write_lock_sor_prox::distribute_work(
                                                                  granular_system const & sys,
                                                                  std::vector<collider::contact> const & contacts,
 ...
 *  for a single worker
 */
void chaotic_parallel_contact_read_write_lock_sor_prox::setup_bcsr_gij_seq(index_t sub_id, sub_problem & wp, granular_system const & sys) {
...
vec3 chaotic_parallel_contact_read_write_lock_sor_prox::solve_one_contact_problem_alart_curnier(
...
//hardware "yield" but not as expensive
//as the os doesn't have to interfere
#ifndef cpu_relax
#define cpu_relax() asm volatile("pause\n": : :"memory")
#endif

//ensures that the store buffer is flushed?
//
#ifndef mem_store_barrier
#define mem_store_barrier() asm volatile("sfence": : :"memory")
#endif

//makes sure, that the compiler
//does not change the order of execution
#ifndef compile_barrier
#define compile_barrier() asm volatile("": : :"memory")
#endif

bool chaotic_parallel_contact_read_write_lock_sor_prox::work_function(
                                                                sub_problem & sub,
                                                                index_t & l_i,
                                                                index_t & g_i,
                                                                index_t l_end,
                                                                std::vector<index_t> & generations,
  ...
  for(; l_i < l_end; ++l_i, ++g_i) {
    ...
    index_t generation_i = generations[g_i];
    vec3 rhs = ci.c;
    ...
    for(index_t j = cbegin; j < cend; ++j) {
      ...
      //read lock contact
      /*maybe we could wait a little and jump to the next contact
       if we have to wait too long?*/
      {
        boost::shared_lock<boost::shared_mutex> lock(mutexes[g_j]);
      
        //make sure we load it every time from memory
        index_t generation_j = (volatile index_t const)(generations[g_j]);
      
        gen_j = std::max(gen_j, generation_j);
        rhs = rhs + sub.gij_blocks[j] * percussions[g_j];
      }
    }
    
    //if not at least one contact has new information
    //then skip to the next contact
    /*if(gen_j < generation_i)
      continue;*/
    ...
    //write lock contact
    {
      boost::unique_lock<boost::shared_mutex> lock(mutexes[g_i]);
      percussions[g_i] = pnew;
      //update contact generation
      generations[g_i] = ++generation_i;
    }
...
chaotic_parallel_contact_read_write_lock_sor_prox::prox_worker::prox_worker(
                                                                      sub_problem & sub_, std::vector<index_t> & generations_, std::vector<vec3> & percussions_,
                                                                      boost::shared_mutex * mutexes_,
                                                                      ...
                                                                      ) : sub(sub_), generations(generations_), percussions(percussions_), mutexes(mutexes_),
percussion_offset(percussion_offset_),
tol_rel(tol_rel_), tol_abs(tol_abs_), max_local_iterations(max_local_iterations_),
diverged(diverged_), done(done_) { }

void chaotic_parallel_contact_read_write_lock_sor_prox::prox_worker::operator()() {
  sub.iteration         = 0;
  sub.converged_counter = 0;
  do {
    //start new iteration
    ...
    bool l_diverged = work_function(sub, l_i, g_i, sub.contacts.size(), generations, percussions, mutexes, tol_rel, tol_abs, max_local_iterations);
...
chaotic_parallel_contact_read_write_lock_sor_prox::prox_master::prox_master(std::vector<sub_problem> const & work_,
                                                                      sub_problem & sub_, std::vector<index_t> & generations_, std::vector<vec3> & percussions_,
                                                                      boost::shared_mutex * mutexes_,
                                                                       index_t percussion_offset_,
                                                                      ...
                                                                      ) : work(work_), sub(sub_), generations(generations_), percussions(percussions_), mutexes(mutexes_),
percussion_offset(percussion_offset_),
...
void chaotic_parallel_contact_read_write_lock_sor_prox::prox_master::operator()() {
  done      = false;
  converged = true;
  diverged  = false;
  iteration = 0;
...
prox_result chaotic_parallel_contact_read_write_lock_sor_prox::run() {
  ...
  for(w = 0; w < m_worker_count - 1; ++w) {
    workers.create_thread(
                          prox_worker(
                                      m_work[w], m_generations, m_percussions, m_mutexes, m_sub_offsets[w],
                                      m_tol_rel, m_tol_abs,
...
void chaotic_parallel_contact_read_write_lock_sor_prox::setup_contacts(
                                                                 granular_system const &                 sys,
                                                                 std::vector<collider::contact> const &  contacts,
  ...
  m_generations.clear();
  m_generations.resize(contacts.size(), 0);
  
  //allocate per contact read-write mutex
  if(m_mutexes)
    delete[] m_mutexes;
  m_mutexes = new boost::shared_mutex[contacts.size()];
}

void chaotic_parallel_contact_read_write_lock_sor_prox::apply_percussions(granular_system & sys) {
