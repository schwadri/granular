//
//  multicolor_parallel_contact_iteration.cpp
//  granular
//
//  Created by Adrian Schweizer on 7/22/12.
//  Copyright (c) 2012 ETHZ. All rights reserved.
//

#include <iostream>

#include "chaotic_parallel_contact_iteration.hpp"

chaotic_parallel_contact_iteration_sor_prox::chaotic_parallel_contact_iteration_sor_prox(
...
void chaotic_parallel_contact_iteration_sor_prox::insert_into_independent_sets(
...
void chaotic_parallel_contact_iteration_sor_prox::build_independent_contact_sets(
...
void chaotic_parallel_contact_iteration_sor_prox::collider_contact_to_solver_contact(
...
void chaotic_parallel_contact_iteration_sor_prox::distribute_work(
                                                granular_system const & sys,
                                                std::vector<collider::contact> const & contacts,
 ...
 *  for a single worker
 */
void chaotic_parallel_contact_iteration_sor_prox::setup_bcsr_gij_seq(index_t sub_id, sub_problem & wp, granular_system const & sys) {
...
vec3 chaotic_parallel_contact_iteration_sor_prox::solve_one_contact_problem_alart_curnier(
...
bool chaotic_parallel_contact_iteration_sor_prox::work_function(
                                              sub_problem & sub,
                                              index_t & l_i,
                                              index_t & g_i,
                                              index_t l_end,
  ...
  for(; l_i < l_end; ++l_i, ++g_i) {
    ...
    //make sure that the new percussion is written to
    //memory before the generation is updated
    mem_store_barrier();
...
chaotic_parallel_contact_iteration_sor_prox::prox_worker::prox_worker(
                                                    sub_problem & sub_, std::vector<index_t> & generations_, std::vector<vec3> & percussions_, index_t percussion_offset_,
                                                    real tol_rel_, real tol_abs_, index_t max_local_iterations_, 
                                                    volatile bool & diverged_, volatile bool & done_
                                                    ) : sub(sub_), generations(generations_), percussions(percussions_), percussion_offset(percussion_offset_),
tol_rel(tol_rel_), tol_abs(tol_abs_), max_local_iterations(max_local_iterations_),
diverged(diverged_), done(done_) { }

void chaotic_parallel_contact_iteration_sor_prox::prox_worker::operator()() {
  sub.iteration         = 0;
  sub.converged_counter = 0;
  do {
    //start new iteration
    ...
    bool l_diverged = work_function(sub, l_i, g_i, sub.contacts.size(), generations, percussions, tol_rel, tol_abs, max_local_iterations);
...
chaotic_parallel_contact_iteration_sor_prox::prox_master::prox_master(std::vector<sub_problem> const & work_,
                                                    sub_problem & sub_, std::vector<index_t> & generations_, std::vector<vec3> & percussions_, index_t percussion_offset_,
                                                    ...
                                                    ) : work(work_), sub(sub_), generations(generations_), percussions(percussions_), percussion_offset(percussion_offset_),
...
void chaotic_parallel_contact_iteration_sor_prox::prox_master::operator()() {
  done      = false;
  converged = true;
  diverged  = false;
  iteration = 0;
...
prox_result chaotic_parallel_contact_iteration_sor_prox::run() {
  ...
  for(w = 0; w < m_worker_count - 1; ++w) {
    workers.create_thread(
                          prox_worker(
                                      m_work[w], m_generations, m_percussions, m_sub_offsets[w],
                                      m_tol_rel, m_tol_abs,
...
void chaotic_parallel_contact_iteration_sor_prox::setup_contacts(
                                               granular_system const &                 sys,
                                               std::vector<collider::contact> const &  contacts,
...
void chaotic_parallel_contact_iteration_sor_prox::apply_percussions(granular_system & sys) {
