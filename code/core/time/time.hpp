...
namespace core {
    ...
    {
    public:
        typedef std::tr1::uint64_t       sec_type;   ///< the type used to represent seconds
        typedef std::tr1::uint64_t       nsec_type;  ///< the type used to represend nanoseconds
      sec_type sec;
      nsec_type nsec;
        ...
        inline operator timeval()
        {
            ...
            if(tv.tv_usec >= time::USEC_PER_SEC)
            {
                tv.tv_sec += tv.tv_usec / time::USEC_PER_SEC;
